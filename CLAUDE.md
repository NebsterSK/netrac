...
- Do not change CLAUDE.md section `laravel-boost-guidelines`, it is auto generated by Laravel Boost.
...
- Use `spatie/laravel-data` DTOs as Inertia view-models: build typed `Data` objects and return them from `Inertia::render` instead of hand-mapped arrays. Place them per module under `app/Data/Finance` and `app/Data/Health`, and annotate each with `#[TypeScript]`.
- Validation stays in FormRequest. When a typed write payload helps, bridge it with `SomeData::from($request->validated())` — the FormRequest still validates (see `MonthlyBalancePayloadData`). Never let a DTO validate the request.
...
- Do **not** run `git add` / `git stage`, `git stash`, `git commit`, or `git push` (or any variant — `git commit --amend`, `git push --force`, etc.) unless the user has explicitly asked for that specific action in the current request.

...
See `README.md` for project overview, stack, setup, and architecture.

## General

- Don't edit the `laravel-boost-guidelines` section (auto-generated).
- Run `php`, `composer`, `npm` via the `.bash_profile` aliases.
- Single-user app — don't scope data by `user_id`.
...
- Use Laravel 13 syntax; declare fillable with the `#[Fillable]` attribute.
...
- Don't run `git add` / `stage` / `stash` / `commit` / `push` (or variants) unless explicitly asked.
